...
- Mobil oturum (**Faz 7B** — [DEM-178](https://linear.app/demirkol/issue/DEM-178)): `apps/mobile` Better Auth Expo client'ı kullanır. Mobilde tarayıcı cookie'si yok — oturum token'ı `expo-secure-store` ile saklanır, session refresh client tarafında yönetilir. sign-in/up/out + şifre sıfırlama akışı web ile aynı Better Auth uçlarına gider; signup bootstrap sunucu tarafında ortaktır (`databaseHooks.user.create.after`) — mobil signup ayrı bootstrap kodu gerektirmez. Mobil auth ekranları → [`08-web-ve-mobil.md`](08-web-ve-mobil.md) §8.2.
