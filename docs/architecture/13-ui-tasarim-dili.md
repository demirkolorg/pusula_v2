...
3. **Başlık satırı** — `flex items-start gap-1.5`: başlık `line-clamp-3 font-medium leading-snug`. _(Not — Faz 2.7B kararı, kullanıcı onayı 2026-05-12: Pusula domain'inde **kart-seviyesi "tamamlandı" özelliği yok** [`cards` tablosunda `completed`/`completedAt` kolonu yok — yalnız `checklist_items.completed`]; bu yüzden `CardCompleteToggle` board kartında **render edilmez** ve "tamamlanmış kart" görünümü [`line-through` vb.] uygulanmaz. `CardCompleteToggle` bileşeni `packages/ui`'de kalır — modal'da/checklist madde checkbox'larında kullanılır. Kart-completion ileride istenirse ayrı domain/backend işi — `faz-baslat`/`bosluk-tara`.)_
