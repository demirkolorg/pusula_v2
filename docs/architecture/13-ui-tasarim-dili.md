...
- **Sticky header** (`px-4 py-2.5 bg-muted/40 backdrop-blur sticky top-0 z-10`): sekme strip `Tabs` — `inline-flex rounded-md border bg-card p-[3px]`; **Yorumlar N** · **Aktivite N** · **Tümü N** (aktif `bg-muted text-foreground`, pasif `text-muted-foreground hover:text-foreground`). _(Ekler sekmesi 2026-07-05'te kaldırıldı — ekler artık sol kolon altındaki collapsible galeride; §13.10.)_
...
- **Liste (sekmeye göre):** Yorumlar = yorum kartları (`Avatar size-sm` + ad + zaman + içerik render + edit/sil hover; silinmiş → "silindi" italic placeholder; düzenlenmişse "(düzenlendi)"). Aktivite = `flex gap-2 text-xs` (actor `Avatar size-xs` + ad + Türkçe özet + zaman + `InfoIcon`). Tümü = yorum + aktivite + ek birleşik kronolojik. _(Ekler sekmesi 2026-07-05'te sağ panelden kaldırıldı — ek yönetimi sol kolon altındaki collapsible galeride; §13.10.)_
...
## 13.10 Kart eki — "Ekler" galerisi (Faz 11D — ✅ wired; 2026-07-05: sağ panel sekmesi → sol kolon collapsible galeri)
...
>
> **Revize (2026-07-05):** Ek yönetimi sağ panel sekmesinden **sol kolon altındaki tam-genişlik collapsible galeriye** taşındı (§13.3 içerik alanı — dikey iki satır; §13.10.9). **Tek kaynak:** sağ paneldeki "Ekler" sekmesi kaldırıldı (Yorumlar / Aktivite / Tümü kaldı). Galeri **salt görüntüleme + yönetim aksiyonlu** (önizle / indir / kapak-yap / sil) — **dosya yükleme galeride değil**, kart başlığındaki "+ Ekle → Ek" popover'ında (`CardAttachmentAddForm` — `Dropzone`, §13.10.1) kalır. Liste satırı `AttachmentTile` yerine galeri kartı `AttachmentGalleryCard` (§13.10.9) kullanır.

Kart detay modalı sol kolonu altındaki "Ekler" galerisinin içeriği. §13.3'te collapsible olarak tanımlı (varsayılan kapalı; header'a tıklayınca açılır, modal geneli scroll çıkmaz — açıkken sol kolon dikeyde 50/50 böler). Implementasyon rehberi → [`08-web-ve-mobil.md`](08-web-ve-mobil.md) §8.1.14; iş kuralları → [`../domain/07-ek-kurallari.md`](../domain/07-ek-kurallari.md); akış → [`09-depolama-ve-arama.md`](09-depolama-ve-arama.md) §9.1.
...
- **Drag-drop kart üstüne dosya bırakma** (Trello davranışı) — yalnız "+ Ekle → Ek" popover'ındaki dropzone V1; sonraki tur (Faz 11.1 — kart üstü dropzone hover overlay).

### 13.10.9 Galeri yerleşimi + `AttachmentGalleryCard` (2026-07-05)

Ekler, sol kolon içerik alanının **alt satırındaki** tam-genişlik panel-card'da yaşar (§13.3). Panel-card AÇIKLAMA/KONTROL LİSTESİ ile aynı kabuk (`flex min-w-0 flex-col overflow-hidden rounded-lg border bg-muted/30`).

- **Collapsible header:** panel-card'ın üst kabuğu tıklanabilir `<button>` (`aria-expanded` + `aria-controls`): solda `PaperclipIcon size-3.5` + "EKLER" (`SectionHeader` tipografisi) + ek sayısı (`text-muted-foreground`), sağda `ChevronDownIcon`/`ChevronUpIcon`. Varsayılan **kapalı** → yalnız bu bar görünür (`shrink-0`). Açıkken panel `flex-1 basis-0 min-h-0` alır, gövde `pusula-scrollbar` ile kendi içinde dikey scroll yapar; üst satır (AÇIKLAMA/KONTROL LİSTESİ) 50/50 karşılığında yarıya iner. Deep-link (bildirim → ek hedefi) galeriyi açılışta açar.
- **Grid:** `grid grid-cols-[repeat(auto-fill,minmax(132px,1fr))] gap-2.5`. Boşken `EmptyState` (§13.10.5).
- **`AttachmentGalleryCard` (yeni `@pusula/ui` bileşeni):** kare hücre (`aspect-square rounded-md border bg-card overflow-hidden group relative`).
  - Görsel (`kind==='image'` + `thumbnailUrl`): `<img object-cover>` tüm hücreyi kaplar (lazy); yoksa/başarısızsa ikon fallback.
  - PDF/Office (`kind==='pdf'|'office'`): §13.10.3 renk paletiyle ikon (pdf `bg-destructive/10 text-destructive`; docx/xlsx/pptx mavi/yeşil/turuncu) — büyük `FileTextIcon`.
  - Alt bant (`absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/70 px-2 py-1`): dosya adı `text-[11px] text-white truncate`.
  - `isCover` → sol üst `bg-primary` rozet "Kapak".
  - Hover/focus overlay (`opacity-0 group-hover:opacity-100 focus-within:opacity-100`): önizle (`EyeIcon`, image+pdf) · indir (`DownloadIcon`) · `DropdownMenu` ⋮ (kapak yap/kaldır [image + member+], sil [uploader/admin, `destructive`]).
  - Önizleme + silme onayı §13.10.4 / mevcut `AttachmentPreviewDialog` + onay `Dialog` ile ortak.
  - Presentational: tüm etiketler `labels` prop (i18n), aksiyonlar callback.
- **Optimistic + realtime:** §13.10.6 aynen geçerli (silme optimistic + rollback; `attachment.list` realtime invalidate). Yükleme galeride olmadığından galeri "yükleniyor" satırı taşımaz.
