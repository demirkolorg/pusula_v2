...
3. **Başlık satırı** — `flex items-start gap-1.5`: başlık `line-clamp-3 font-medium leading-snug`. _(Durum — güncel 2026-05-12: kart-seviyesi "tamamlandı" **backend'i artık var** — `cards.completed`/`completedAt`/`completedBy` + `card.complete`/`card.uncomplete` ([DEM-66](https://linear.app/demirkol/issue/DEM-66)); kapak rengi de `cards.coverColor` + `CARD_COVER_COLORS` ([DEM-67](https://linear.app/demirkol/issue/DEM-67)) — `checklist_items.completed`'tan **bağımsız** (bkz. [`03-backend.md`](03-backend.md) Faz 2.7, [`../domain/01-urun-modeli.md`](../domain/01-urun-modeli.md) invariant 15). Bu spec'teki `CardCompleteToggle` (kartta hover'da, tamamlanmışsa hep görünür: `bg-success` tik + "tamamlanmış kart" görünümü `line-through` vb.) ve kart kapak rengi şeridi (`-mx-2 -mt-2 mb-1.5 h-3 rounded-t-md bg-palet-{ad}`) Faz 2.7B/2.7C'de **henüz wire edilmedi** (2.7B board kartına, 2.7C modal başlığına bağlamadı — kasıtlı erteleme) → board kartı/modal bu mutation'ları kullanmaya başladığında bu spec geçerli. `CardCompleteToggle` bileşeni `packages/ui`'de; modaldaki checklist madde checkbox'larında kullanılır. Wire follow-up: ayrı iş / `bosluk-tara`.)_
