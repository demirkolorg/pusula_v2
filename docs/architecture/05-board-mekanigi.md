...
- **Board detay** (`(app)/workspaces/[id]/boards/[boardId]`): `trpc.board.get` tek seferde board + listeleri (`position` sıralı) + her listenin aktif kartlarını (`position` sıralı) döndürür → kolon + kart render. Kartın tam `description` gövdesi bu özet response'a dahil edilmez; metadata satırı için yalnız `hasDescription` boolean'ı döner. Tam içerik, kart detayı açıldığında `card.get` ile alınır. Her kart ayrıca `labels[]` + `checklistTotal`/`checklistDone` + `commentCount` (`deleted_at IS NULL`) + `members[]` (ad+görsel — avatar yığını; e-posta yok) taşır; hepsi board genelinde toplu sorgu (N+1 yok). UI tarafı bu sayaçlardan kart metadata satırını (due chip + "GECİKTİ" rozeti + amber-soon-dot + açıklama-var + checklist progress + yorum sayısı + üye avatarları) çizer (tasarım dili: [`13-ui-tasarim-dili.md`](13-ui-tasarim-dili.md) §13.2). Kart-completion özelliği yok → kartta "tamamla" toggle'ı render edilmez; attachment sayacı Faz 8.
...
- Büyük board'larda, listede 50+ kart varsa önce kart DOM'unu koruyan
  `content-visibility: auto` ara optimizasyonu kullanılır; PDD kaydı, klavye sırası ve
  drop hedefleri mount kalır. Kart ölçüleri stabil — hover/drag'de layout shift yok.
  Dinamik virtualization ancak 200/1.000 kart benchmark'ı bu basamağın yetersiz olduğunu
  gösterirse eklenir.
