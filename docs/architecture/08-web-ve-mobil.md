...
### 8.1.6 Board üye + etiket yönetimi UI (Faz 2.5E)

Board ayarları/detay ekranına üye/rol yönetimi + etiket yönetimi + board ekranında etiket filtresi. Backend Faz 2.5B (`label.*`) + Faz 2.5C (`board.members.*`, `board.invitations.*`) tamam olmalı. Backend sözleşmesi → [`03-backend.md`](03-backend.md) (Faz 2.5 procedure'leri), [`../domain/02-yetkilendirme-kurallari.md`](../domain/02-yetkilendirme-kurallari.md).

- **Konum:** board detay sayfasında (§8.1.4 Board ekranı) bir "Board ayarları" girişi — board `admin` ise — shadcn `Dialog` veya kenar paneli açar (workspace yönetim ekranındaki §8.1.2 pattern'iyle hizalı). `viewer+` üye listesini görür; aksiyonlar (üye ekle/rol değiştir/çıkar, davet iptal, etiket CRUD) yalnızca board `admin`'e.
- **Üyeler:** `trpc.board.members.list` → satır başına ad/e-posta/rol; inherited (workspace owner/admin) satırlar ayrı işaretli ("workspace yöneticisi — düzenlenemez"). `admin` ise: rol değiştir (shadcn `Select` — `admin`/`member`/`viewer`; son board `admin` ve inherited satır kilitli) → `board.members.updateRole`; çıkar → `board.members.remove` (kendi satırında "board'dan ayrıl"); üye ekle (e-posta input) → `board.members.add` — sonuç: workspace üyesi → doğrudan eklendi / hesabı olan non-member → `guest` yapılıp eklendi / hesabı yok → davet gönderildi (inline durum mesajı; `CONFLICT` → "zaten üye"). Mutation sonrası `board.members.list` (+ gerekiyorsa `board.get`) invalidate.
- **Gönderilmiş board davetleri:** `trpc.board.invitations.list` → bekleyen davetler (e-posta, rol, davet eden, son tarih); `admin` ise "iptal et" → `board.invitations.revoke` → `invitations.list` invalidate. Kullanıcının kendi board davetleri (`board.invitations.mine`) workspace davetleriyle aynı yerde — `(app)/page.tsx` "Bekleyen davetler" bölümü (§8.1.1'deki `PendingInvitations`) genişletilir: hem workspace hem board davetleri listelenir, "Kabul et"/"Reddet" → `board.invitations.accept`/`decline` (`clientMutationId` ile); kabul sonrası `workspace.list` + `board.list` (varsa) + ilgili `invitations.mine` invalidate.
- **Etiketler:** board etiket listesi (`board.get`'in döndürdüğü `labels` ya da `label.list`) → satır başına renk swatch + ad; board `member+`: etiket ekle (renk seçici — `@pusula/domain` izinli renk tokenları + opsiyonel ad) → `label.create`; düzenle → `label.update`; sil → `label.delete` (kartlardan da kalkar — onaylı dialog). `CONFLICT` (aynı renk+ad) inline. Mutation sonrası `board.get`/`label.list` invalidate.
- **Etiket filtresi (board ekranı):** board detay sayfasının üstüne küçük bir "etikete göre filtrele" kontrolü (etiket çipleri — seçili olanlara sahip kartlar gösterilir; client-side filtre, `board.get` verisi üzerinde). **Arşivli öğeler görünümü (G7):** aynı yerde "arşivli liste/kartları göster" toggle'ı — board ekranı normalde yalnızca aktif kartları render eder; toggle açıkken arşivli listeler/kartlar soluk gösterilir ve restore aksiyonu (zaten var: `list.archive`/`card.archive` `archived:false`) erişilebilir olur. (Tam "arşiv kutusu" ekranı isteğe bağlı ileride; bu fazda inline toggle yeterli.)
- **shadcn:** `Dialog`/`Select`/`Button`/`Input`/`Label`/`Badge`/`Alert` mevcut; gerekirse `Popover` (renk seçici), `Switch`/`Checkbox` (arşiv toggle) eklenir. Türkçe metinler `strings.board` (üye/etiket alt grupları); hardcode metin yok. `clientMutationId` istemcide.
- **Test:** React Testing Library — üye listesi (rol-bazlı görünürlük + rol değiştir/çıkar/ekle callback), board davet listesi/iptal, etiket CRUD formu (renk/ad validasyon), etiket filtresi + arşiv toggle (presentational, callback + filtre mantığı). Playwright e2e ileri faz (Faz 8).
