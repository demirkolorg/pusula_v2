...
- **Faz 2.7C ([DEM-64](https://linear.app/demirkol/issue/DEM-64)) — yapısal yeniden kurulum:** bu modal Faz 2.7'de tasarım diline çekilir (bağlayıcı spec → [`13-ui-tasarim-dili.md`](13-ui-tasarim-dili.md) §13.3 + §13.5; ortak bileşenler §13.4). Değişen yapı: shadcn `Dialog` içinde **iki-kolon** (`grid md:grid-cols-[1fr_360px]`), **kapak-renkli başlık çubuğu** (breadcrumb pano/liste + bell/link/⋮/×; kapak rengi DEM-67'den), **sol kolon** = sticky başlık + `CardCompleteToggle` (DEM-66'dan; backend inmeden disabled placeholder) + meta chip satırı (üye/due/etiket/kapak-rengi) + AÇIKLAMA (`RichTextEditor` — Tiptap, §13.5) + KONTROL LİSTESİ (`SectionHeader` + `Progress` + atanan `Avatar`), **sağ panel** ≈360px = `Tabs` sekme strip (Yorumlar/Aktivite/Ekler/Tümü) + yorum composer (`Avatar` + mini `RichTextEditor` + Gönder) + aktivite feed + `EmptyState`; mobilde tek kolon (sağ panel alta düşer). **Backend sözleşmesi değişmez** — `card.update` / `comment.*` procedure imzaları aynı (string in/out); `description`/`body` artık **Tiptap JSON** olarak saklanır ama `text` kolonu değişmez, migration yok, mevcut plain-text içerikler render/edit anında düz paragrafa parse edilir (parse-time fallback; §13.5 + [`02-teknoloji-kararlari.md`](02-teknoloji-kararlari.md) Karar kaydı 2026-05-12). Optimistic UI yine yok (Faz 4); `clientMutationId` istemcide.
