...
> **Düzeltme (2026-07-22 — transaction/channel handoff yarışı):** `notification-publish`
> processor'ı email/push alt işlerini outbox satırlarını kilitlediği DB transaction'ın
> içinden enqueue etmez. Kanal outbox id'leri transaction içinde yalnız tamponlanır;
> BullMQ handoff **COMMIT'ten sonra** yapılır. Aksi halde kanal worker'ı erken başlayıp
> `FOR UPDATE SKIP LOCKED` nedeniyle satırı "missing" sanabilir, işi başarılı kapatabilir
> ve satır günlerce pending kalabilir. Kanal processor'ı pending ama kilitli satırı gerçek
> "missing" saymaz; retry için hata verir. Sweeper recovery işleri retained completed/failed
> `jobId` tarafından engellenemez: recovery enqueue benzersiz iş kimliği kullanır; kanal
> handoff'u ise DB'de hâlâ pending olan satıra ait tamamlanmış/failed child job'u temizleyip
> aynı deterministik kimlikle yeniden kurabilir. `processed_at` kalıcı idempotency çapasıdır.
...
> **Wired (2026-05-16 — [DEM-175](https://linear.app/demirkol/issue/DEM-175)):** Board'a doğrudan üye eklemesi kendi bildirim tipine ayrıldı. `board.member_added` activity'si Faz 2.5'ten beri `board_invitation` tipiyle bildiriliyordu — "Pano daveti / …davet etti" metni + davet kabul/reddet beklentisi yanıltıcıydı (kullanıcı zaten üye) ve `board_invitation` mute-bypass olduğundan susturmuş kullanıcı yine de anlık e-posta alıyordu. `NOTIFICATION_TYPES`'a `board_member_added` append edildi (migration `0032_dem175_board_member_added` — `notification_type` pg enum'a tek `ADD VALUE IF NOT EXISTS`; `ACTIVITY_EVENT_TYPES` değişmez — `board.member_added` zaten vardı). `mapEventToNotificationType` `board.member_added` → `board_member_added`; kanal `pickChannels` in_app + email opt-in (`emailByType`), push yok, **mute-bypass değil** (gerçek token'lı `board_invitation` mute-bypass kalır). Worker `notification-templates.ts` email/push/digest switch'lerine `board_member_added` case ("davet etti" değil "ekledi"). Web: `notification-type-icon` / `activity-summary` + `strings` / `notifications-shared.ts MATRIX_ROWS` yeni tip. Davet kabulü de `board.member_added` üretir ama actor = yeni üye → self-skip bildirim doğurmaz. Domain tarafı → [`../domain/04-bildirim-kurallari.md`](../domain/04-bildirim-kurallari.md) "DEM-175 — doğrudan board üyeliği bildirimi".
...
- **Worker processor (`apps/worker/src/jobs/notification-publish.ts`):**
  1. Event'e ait pending `notification_outbox` satırlarını kısa transaction'da kilitle (`FOR UPDATE SKIP LOCKED`).
  2. Channel'a göre transaction-içi hazırlık:
     ...
     - `email` / `push` → outbox id'sini post-commit handoff tamponuna ekle; transaction içinde BullMQ enqueue **yapma**, `processed_at` damgalama.
  3. COMMIT sonrasında tamponlanan id'leri `pusula-notifications-email` / `pusula-notifications-push` kuyruğuna aktar. Kanal processor gerçek gönderim/skip sonrasında `status='sent'` damgalar.
  4. Email/push enqueuer eksik veya geçici olarak hatalıysa satır başarılı sayılmaz: `processed_at` boş, `status='pending'`, `last_error` dolu ve `attempts+1` kalır; parent job hata verip BullMQ retry'a girer, sweeper da son kurtarma katmanıdır.
  5. Kanal processor `SKIP LOCKED` sonrasında satır bulamazsa ikinci, kilitsiz bir durum sorgusuyla ayrım yapar: satır yok/zaten processed → idempotent no-op; satır pending → lock contention, job fail + retry.
  6. In-app hatası → BullMQ retry (queue default attempt/backoff); nonsensical `in_app` satırları (recipient yok) `status='dead'` ile kapatılır.
- **Periyodik sweeper (`pusula-notifications-sweeper`):** `processed_at IS NULL AND created_at < NOW() - INTERVAL '30 seconds'` satırları yeniden enqueue eder (60s aralıklı cron). Recovery job'u retained bir completed/failed `notify-{eventId}` ile dedupe edilmez; benzersiz BullMQ id'siyle gerçek bir yeni deneme oluşturur. DB lock + `processed_at` tekrar teslimi engeller.
...
- **Worker processor (`apps/worker/src/jobs/notification-email.ts`):**
  ...
  4. Outbox `created_at` değeri 24 saatten eskiyse transactional e-posta gönderme; satırı `status='dead'`, `last_error='email_stale'` ile kapat. In-app kayıt kalır; digest satırlarını bu processor işlemez.
  5. Template seç (notification type'ına göre — atama / mention / yorum / due / davet). HTML template `packages/email/templates/` veya inline JSX.
  6. `resend.emails.send({ from: EMAIL_FROM, to, subject, html })`.
  7. `notification_outbox.processed_at = NOW()` + `status='delivered'`.
  8. Hata (Resend rate limit / network) → BullMQ retry (3 attempt); dead-letter.
...
- **Worker processor (`apps/worker/src/jobs/notification-push.ts`):**
  ...
  5. Outbox `created_at` değeri 15 dakikadan eskiyse fiziksel push gönderme; satırı `status='dead'`, `last_error='push_stale'` ile kapat. In-app bildirim kalıcı kayıt olarak durur.
  6. Token VAR → Expo Push API'ye batch request (`https://exp.host/--/api/v2/push/send`; `expo-server-sdk` Node.js client). Payload: `{ to: token, title, body, ttl: 900, data: { type, cardId?, boardId? } }`; Expo/APNs tarafında da aynı 15 dakikalık tazelik penceresi uygulanır.
  7. Response: `DeviceNotRegistered` / `InvalidCredentials` hata → `push_tokens.revoked_at = NOW()` damgalanır (audit; silinmez).
  8. `notification_outbox.processed_at = NOW()` + `status='delivered'`.
  9. Hata → BullMQ retry (3 attempt); dead-letter.
