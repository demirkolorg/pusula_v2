...
> **DEM-98 (Trello-stili liste rengi) kapsamı:** `lists` tablosuna `color text` kolonu eklenir (nullable; `null` = mevcut `bg-muted/30` varsayılan kolon zemini). Değer seti `@pusula/domain` `LIST_COLORS` paletindeki 10 adla sınırlıdır (`yesil/sari/turuncu/kirmizi/mor/mavi/sky/lime/pembe/gri`); DB-CHECK yok, doğrulama Zod/API katmanında yapılır. Aynı migration `ACTIVITY_EVENT_TYPES` enum'una `list.color_changed` ve `list.color_cleared` değerlerini append eder (`packages/db/src/migrations/00XX_*_lists_color*.sql` — DEM-98 migration referansı; kesin sıra migration üretiminde sabitlenir). Procedure haritası → [`03-backend.md`](03-backend.md); activity payload'ları → [`../domain/05-aktivite-kurallari.md`](../domain/05-aktivite-kurallari.md); realtime payload'ı → [`05-board-mekanigi.md`](05-board-mekanigi.md) §5.3; domain invariant'ı → [`../domain/01-urun-modeli.md`](../domain/01-urun-modeli.md) invariant 17.
