...
| `label` | `create` | `boardProcedure` | board `member+` (`canEditBoardContent`); board scope'lu etiket (`name` opsiyonel, `color` zorunlu — `@pusula/domain` izinli renk tokenları); arşivli board salt-okunur; `(boardId,color,name)` benzersiz → çakışma `CONFLICT`; **activity yok** (board metadata); `boards.version` artar |
...
| `board.members` | `list` | `boardProcedure` | board `viewer+`; explicit `board_members` satırları (ad/e-posta/rol) + (gösterim için) workspace owner/admin'lerin inherited erişimi UI'da ayrı işaretlenir |
