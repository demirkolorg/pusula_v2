...
- **Permission check:** Bildirim alıcısı olay anında ilgili kaynağa erişebilmelidir (board üyesi değilse / silinmişse → bildirim üretilmez). Bu kural **due-date scheduler** hatırlatmaları (`due_reminder_1d/1h`, `due_overdue`) için de geçerlidir (2026-07-20): alıcı havuzu `card_members` olsa da, efektif pano erişimi kalmamış kullanıcı (workspace üyeliği kalkmış **veya** `guest` olup explicit pano koltuğu olmayan) filtrelenir. Erişim kaybı akışları kalıntı `card_members` satırlarını zaten aynı transaction'da düşürür (invariant 24 → [`01-urun-modeli.md`](01-urun-modeli.md)); buradaki üretim-anı kontrolü o temizliğin **savunma katmanıdır** (geçmiş kalıntılar + öngörülemeyen akışlar için).
