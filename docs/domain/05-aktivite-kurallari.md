...
| List | `list.created`, `list.renamed`, `list.archived`, `list.restored`, `list.moved`, `list.color_changed`, `list.color_cleared`, `list.icon_changed`, `list.icon_cleared` | eski/yeni başlık, eski/yeni position, `{ listId, oldColor, newColor }` / `{ listId, oldColor }`, `{ listId, oldIcon, newIcon, oldIconColor, newIconColor }` / `{ listId, oldIcon, oldIconColor }` |
...
>
> **Liste ikonu ve ikon rengi (DEM-109):** `list.update({ icon, iconColor })` ikon veya ikon renginde gerçek değişiklik varsa activity üretir. İkon ekleme/değiştirme ve yalnız renk değişimi/sıfırlama `list.icon_changed` üretir; payload `{ listId, oldIcon, newIcon, oldIconColor, newIconColor }`. İkon kaldırma (`icon: null`) `list.icon_cleared` üretir; payload `{ listId, oldIcon, oldIconColor }` ve server `iconColor`'ı da `null` yapar. Aynı ikon/renk tekrar set/clear idempotent no-op'tur; activity ve version üretmez. Bu iki event tipi `ACTIVITY_EVENT_TYPES`'a **append** edilir ve `lists.icon`/`lists.icon_color` migration'ıyla aynı PR'dadır. Bkz. [`01-urun-modeli.md`](01-urun-modeli.md) invariant 18.
