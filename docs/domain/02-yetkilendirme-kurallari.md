...
| `board.accessRequests.request` | `protectedProcedure` | (oturum) | Board-scope erişim talebi oluşturur; aynı `(boardId, requesterId)` için tek `pending` satır; caller zaten board `viewer+` ise no-op sonucu döner. Workspace için ayrı talep tipi yoktur. |
