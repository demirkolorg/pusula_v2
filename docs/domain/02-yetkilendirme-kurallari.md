...
- **Board rolleri (admin / member / viewer):** `admin` = board'u yönetir (ayarlar, arşiv, üye/rol, etiket dahil her şey); `member` = içerik düzenler (liste/kart/checklist/yorum/etiket-atama/kart-üyesi) ama board'u yönetemez; `viewer` = salt-okunur (yalnızca kendisinin mevcut `assignee`/`watcher` satırını bırakabilir — başkası onu eklemiş olmalı).
- Card rolleri yetki seviyesi değil, **ilgi/ilişki** belirtir: `assignee` (kart kendisine atanmış), `watcher` (kartı izliyor — bildirim alır). Kart üzerinde düzenleme yetkisi board rolünden gelir. Karta üye atanan kişi o board'a erişebilen biri olmalı (`effectiveBoardRole !== null`) — aksi halde "atanmış ve göremeyen" kullanıcı oluşur.
- **Self-add yasağı (DEM-298, 2026-05-25):** Kullanıcı **kendisini** karta üye yapamaz (assignee veya watcher fark etmez), board'a ekleyemez ve workspace'e davet edemez. Niyet: "kart üyeliği bir ilişki sözleşmesidir — başkası seni eklemeli; kendini eklemek bilgi taşımaz". Self-watch (eski "Bu kartı izle" toggle'ı) kaldırıldı — birinin seni watcher yapması veya assignee yapması beklenir; istemediğin atama/izlemeyi kendin bırakabilirsin (`card.members.remove` self-leave hâlâ açık).
...
| `workspace.members.updateRole` | admin+      | `owner` rolü atanamaz/kaldırılamaz — owner devri ayrı akış. `guest`'e düşürmede kullanıcının explicit koltuğu olmayan panolardaki kart üyelikleri aynı tx'te silinir (invariant 24)                                                                                                            |
| `workspace.members.remove`     | admin+      | Üye kendini çıkarabilir; son `owner` çıkarılamaz. Çıkarılanın bu workspace panolarındaki kart üyelikleri aynı tx'te silinir (invariant 24)                                                                                                                                                    |
| `workspace.members.invite`     | admin+      | `workspace_invitations`'a `pending` satır + `notification_outbox` (email; alıcı hesaplıysa in-app de) yazar — aşağıdaki davet akışı. **Caller kendi e-postasını giremez** (`BAD_REQUEST` "Kendinizi davet edemezsiniz." — DEM-298).                                                          |
...
| Procedure                       | Gereken rol                                | Not                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| ------------------------------- | ------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `workspace.members.invite`      | admin+ (`workspaceProcedure`)              | `email` küçük harfe normalize; **caller'ın kendi e-postası ise `BAD_REQUEST` ("Kendinizi davet edemezsiniz." — DEM-298)**; davet edilen zaten üye ise `CONFLICT`; (workspace, email) için `pending` davet zaten varsa `CONFLICT`; rol `owner` olamaz (`assignableWorkspaceRoleSchema`, varsayılan `member`). `workspace_invitations` insert + `activity_events` (`workspace.member_invited`) + `notification_outbox` (`workspace_invitation`, channel `email`; davet edilenin hesabı varsa ek `in_app` satır, `recipient_id` o kullanıcı) — hepsi aynı transaction'da. |
...
| Mevcut atamayı / izleyiciliği (kendi `assignee`/`watcher`) bırak³  | ✓           | ✓            | ✓            |
| Board'u favorile / favorisinden çıkar (kişisel)                    | ✓           | ✓            | ✓            |

> ¹ Son board `admin` board'dan ayrılamaz / rolden düşürülemez. ² Yorum düzenleme/silme: yazan kişi **veya** board `admin`; `viewer` yorum bile ekleyemez. ³ Kart üyeliği başkası tarafından eklenir; kullanıcı yalnızca kendi mevcut `assignee`/`watcher` satırını silebilir — kendini ekleyemez (DEM-298). **Board favorisi kişiseldir** — board görüntüleme erişimi (`viewer+`) yeterli; ayrı yetki kuralı yoktur, activity/realtime üretmez (bkz. [`01-urun-modeli.md`](01-urun-modeli.md) invariant 19).
...
| `board.moveToWorkspace` (2026-07-13) | `boardProcedure` | board `admin` (`canManageBoard`) **ve** hedef workspace'te `member+` (`guest` hariç — `board.create` ile aynı kural) | Panoyu **tüm içeriğiyle** (liste/kart/etiket/checklist/yorum/ek/üye/davet) başka workspace'e taşır — yalnız `boards.workspace_id` değişir; board-scope child satırlar `board_id` üzerinden bağlı olduğu için kendiliğinden gelir. Arşivli board → `BAD_REQUEST`; arşivli hedef workspace → `BAD_REQUEST`; hedef workspace'e erişim yoksa `FORBIDDEN`; hedef = mevcut workspace → no-op (`changed: false`, idempotent — `clientMutationId`). Board üyeleri **korunur**; hedef workspace'te üyeliği olmayan explicit board üyeleri aynı tx'te otomatik workspace `guest` yapılır (board davet kabulü / erişim talebi onayı emsali — invariant 13). Denormalize `workspace_id` alanları aynı tx'te yeni workspace'e taşınır: `activity_events` (board'un geçmişi board ile gider), `notifications` (kolon **+ payload `workspaceId`** — deep-link payload'dan kurulur, kolon FK cascade doğruluğu için), `share_links` (board'un kartlarına ait), `search_documents` (scope re-sync). `notification_outbox` scope kolonu taşımaz (scope payload'dadır; pending satır penceresindeki bayat deep-link'i board ekranının canonical redirect'i telafi eder). `audit_log` tarihsel kayıt olarak **taşınmaz**; kaynak workspace'in saved report konfigürasyonları board'u runtime'da doğal olarak kapsam dışı bırakır (migration yok). `activity_events` (`board.moved_workspace` — `fromWorkspaceId`/`toWorkspaceId` + workspace adları); **bildirim üretmez** (v1 — `mapEventToNotificationType` null döner). `boards.version` artar; realtime `board.movedToWorkspace` board odasına yayınlanır (açık board ekranları URL/cache günceller). |
...
| `list.delete`   | `boardProcedure`                                                         | board `admin` (`canManageBoard`)        | **Faz 17 (2026-06-01)** — kalıcı silme. Yalnızca boş liste (aktif + arşivli kart yok); arşivli board reddedilir. `lists` satırı silinir; cascade FK kuralı `lists.id` referans alan child satırları temizler (liste boş olduğu için pratikte yalnız `search_documents` etkilenir). Activity (`list.deleted`, `cardId/listId` `null` — silinen id `payload.listId`'de) + realtime event aynı tx; `boards.version` artar. Idempotent: bilinmeyen `listId` → `{ id, changed: false }`. **Geri alınamaz** (arşivlemenin aksine restore yok). |
| `card.delete`   | `cardProcedure`                                                          | board `admin` (`canManageBoard`)        | **Faz 17 (2026-06-01)** — kalıcı silme. Cascade FK kuralları `card_members`/`card_labels`/`checklists`/`checklist_items`/`comments`/`attachments`/`activity_events`/`realtime_events`/`notifications`/`share_links`/`search_documents` satırlarını temizler. Attachment MinIO blob'ları silmeden önce yakalanır, commit sonrası `pusula-attachment-cleanup` BullMQ kuyruğuna best-effort enqueue edilir (60 dk sweeper safety net). Activity (`card.deleted`, `cardId` null — silinen id `payload.cardId`'de) + realtime event aynı tx; `boards.version` artar. Arşivli kart yine silinebilir; arşivli board reddedilir. **Geri alınamaz.** |

Activity: `board.created/renamed/archived`, `list.created/renamed/archived/deleted`, `card.created/renamed/description_changed/due_set/due_cleared/archived/deleted` ilgili transaction'da `activity_events`'e yazılır (bu tipler [`05-aktivite-kurallari.md`](05-aktivite-kurallari.md) taksonomisinde zaten tanımlı; Faz 2'de bu alt küme `ACTIVITY_EVENT_TYPES`'a eklenir). Realtime yayın Faz 5, bildirim outbox Faz 6.
...
| `card.members.add`                      | `cardProcedure`      | board `member+` — kullanıcı kendisini ekleyemez (DEM-298)               | `input.userId === caller` ise `FORBIDDEN` ("Kendinizi karta ekleyemezsiniz."); aday `effectiveBoardRole !== null` olmalı (aksi `BAD_REQUEST`); rol `assignee`/`watcher`; idempotent; `card.member_added`                                                        |
| `card.members.remove`                   | `cardProcedure`      | board `member+` (kendini `watcher`/atamadan çıkarma `viewer`'a da açık) | İdempotent; `card.member_removed`. Self-leave kuralı korunur (kullanıcı kendi `assignee`/`watcher` satırını silebilir).                                                                                                                                         |
...
| `board.members.add`                     | `boardProcedure`     | board `admin` — caller kendi e-postasını giremez (DEM-298)              | `email` ile; caller'ın kendi e-postası ise `BAD_REQUEST` ("Kendinizi davet edemezsiniz."); workspace üyesini doğrudan ekler / hesabı olan non-member'ı `guest` yapıp ekler / hesabı yoksa `board_invitations` daveti; `board.member_added` (+ `workspace.member_added`) ya da `board.member_invited` |
...
| `board.members.remove`                  | `boardProcedure`     | board `admin` (üye kendini = board'dan ayrıl)                           | Yalnızca explicit satır; son board `admin` çıkarılamaz; `board.member_removed`. Geriye efektif erişim kalmadıysa (ws `guest` + koltuk yok) bu panodaki kart üyelikleri aynı tx'te silinir (invariant 24)                                                          |
...
| `list.moveToBoard` (2026-07-14) | `boardProcedure` (kaynak) | kaynak board `member+` **ve** hedef board'da `member+` (`resolveBoardAccess`) | Listeyi **tüm kartlarıyla** (aktif + arşivli) başka panoya taşır — cross-workspace dahil; liste hedef panonun **sonuna** eklenir. Kaynak/hedef board arşivli veya listenin kendisi arşivli → `BAD_REQUEST`; hedef = mevcut board → no-op (`changed: false`; aynı-board sıralama `list.move`). Kartların `cards.board_id`'si güncellenir; kartların `card_labels` satırları **silinir** (board-scope — `card.moveToList` simetrisi); `card_members`/checklist/yorum/ek/activity kartla gelir. `activity_events` (`list.moved` — payload additive: `title` + `fromBoardId`/`toBoardId` + board başlıkları; **yeni activity tipi yok**); bildirim mevcut `list_moved` tipiyle hedef board audience'a. İki board `version++`; realtime `list.movedToBoard` **iki** board odasına yayınlanır (worker cross-board fanout — `card.movedToList` ile aynı mekanizma). Hedef board search scope re-sync. İdempotent (`clientMutationId`; liste zaten hedef board'daysa `changed: false`). |
| `list.moveAllCards` (2026-07-14) | `boardProcedure` | board `member+` (`canEditBoardContent`) | Bir listedeki **tüm aktif** kartları **aynı board içinde** başka listenin sonuna toplu taşır (Trello "Move all cards in this list"). Kaynak/hedef aynı board'da olmalı; hedef arşivli → `BAD_REQUEST`; kaynak = hedef **veya** kaynak boş → no-op (`changed: false`, `movedCount: 0`). Kartların mevcut sırası korunur (hedef sonuna ardışık `positionsBetween`). **Düşük-sinyal toplu işlem** — `card.moved` activity **yazılmaz** ve bildirim üretilmez (position compaction / etiket CRUD emsali; kullanıcı tek tek taşırsa activity+bildirim olur). Aynı-board taşıma olduğu için `card_labels` **korunur**. `boards.version++`; realtime `list.cardsMoved` board odasına (izleyiciler refetch eder). İdempotent (`clientMutationId`). Cross-board toplu taşıma kapsam dışı — tüm listeyi taşımak için `list.moveToBoard`. |
...
>
> **Pano taşıma (2026-07-13):** Kartın aksine _panoyu_ başka workspace'e taşımak (`board.moveToWorkspace`, yukarıdaki CRUD haritası) hedef tarafta **workspace üyeliği** ister: board `admin` + hedef workspace `member+` (`guest` hariç) — çünkü işlem hedef workspace'te yeni bir board var etmekle eşdeğerdir (`board.create` simetrisi). Taşınan panonun hedef workspace'te üyeliği olmayan board üyeleri otomatik workspace `guest` yapılır; board rolleri değişmez.
