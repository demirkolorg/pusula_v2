...
## 5. Paralel tab haritası (anlık — 2026-05-24, Faz 8.0 önce-belge sonrası)
...
| **Faz 8B — Realtime fix (kod tab'ı, sıradaki açılacak)** | [DEM-278](https://linear.app/demirkol/issue/DEM-278) (`setupSocketServer` await + `waitForSocketJoin` kaldır — DEM-86 follow-up) | `Todo` — küçük P0; DEM-277 Done sonrası 8.0 önce-belge yol açtı. 8A E2E suite genişletmesi bunu bekler. | `apps/api/src/index.ts`, `apps/api/src/socket/`, `e2e/realtime-board-sync.spec.ts`, `e2e/fixtures/realtime.fixture.ts` |
