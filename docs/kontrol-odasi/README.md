...
| Faz 3 tab'ları (3A/3B/3C/3D/3E) | `list.move`/`card.move` backend ([DEM-42](https://linear.app/demirkol/issue/DEM-42)) → drag-drop UI web ([DEM-43](https://linear.app/demirkol/issue/DEM-43)) ∥ position compaction worker ([DEM-44](https://linear.app/demirkol/issue/DEM-44)) → Playwright drag-drop testleri ([DEM-45](https://linear.app/demirkol/issue/DEM-45)); `card.moveToList` (cross-board) + `card.copy` ([DEM-69](https://linear.app/demirkol/issue/DEM-69) — Faz 3E, 3A sonrası) | **Geliştirme 2026-05-13 başladı (`faz-baslat 3`)** — 3.0 önce-belge ([DEM-41](https://linear.app/demirkol/issue/DEM-41) / `DOC-2026-05-13-001`) `In Progress` (**bu kontrol odası tab'ında** — `05-board-mekanigi §5.1` + `03-siralama-kurallari` + `@pusula/domain` move-input şekilleri tamam; kalan `08-web §8.1.8` board-drag-drop, DEM-74 land sonrası); 3A ([DEM-42](https://linear.app/demirkol/issue/DEM-42)) `Todo` — **DEM-74 ile paralel açılabilir** (`packages/api/db/domain` ↔ DEM-74'ün `apps/web` dosyaları çakışmaz); 3B ([DEM-43](https://linear.app/demirkol/issue/DEM-43)) `Todo` — `apps/web` board dosyaları DEM-74 ile çakışıyor → DEM-74 land + 3A bekler; 3C ([DEM-44](https://linear.app/demirkol/issue/DEM-44)) `Todo` — 3A bekler, DEM-74 ile çakışmaz; 3D ([DEM-45](https://linear.app/demirkol/issue/DEM-45)) `Todo` — 3B bekler; 3E ([DEM-69](https://linear.app/demirkol/issue/DEM-69)) `Todo` — 3A bekler, DEM-74 ile çakışmaz | `packages/api`, `packages/domain`, `packages/db` (3A — gerekirse migration), `apps/worker` (3C), `apps/web` (3B/3D — board kolon bileşenleri/`card-item.tsx`/drag handler'ları/Playwright), `docs/architecture/03,05,06,08` (kod tab'larının kendi turlarında), `docs/domain/01,02,03` |
| **Bu tab (kontrol odası)** | Süreç projeksiyonu + `docs/` + Linear senkronu | sürekli açık; son turlar: `bosluk-tara` 2. tur → 8 yeni issue DEM-66–73; DEM-74 (2.7C-2) tarayıcı görseli ile açıldı + tab prompt'u; `linear-senkron` — DEM-74 → `In Progress`; **`faz-baslat 3` (2026-05-13)** — Faz 3 geliştirmesi başlatıldı: `05-board-mekanigi §5.1` yeniden yazıldı (UI ilkeleri + `moveCard`/`moveList` şekilleri + server move akışı + faz kapsamı + test) + `03-siralama-kurallari` tamamlandı (compaction tetiği `POSITION_COMPACTION_MAX_LEN≈50` + `CONFLICT` semantiği + cross-board `card.moveToList` notu) + faz planı (Faz 3 satırı `🚧` + footer + "Faz 3 alt işleri" 1–6 [DEM-69 madde 6 olarak eklendi]) + iş kayıt defteri (DEM-41 `DOC-2026-05-13-001` `In Progress` + DEM-42–45/69 `API/FE-2026-05-13-*` `Todo` + footer) + bu README §5 (Faz 3 tab'ları satırı) güncellendi; Linear DEM-41 → `In Progress`, DEM-42/43/44/45/69 → `Todo`. `@pusula/domain` move-input şemaları (`moveCardInput`/`moveListInput`) zaten mevcut & doğru — kod yazılmadı. **Kalan:** `08-web §8.1.8` board-drag-drop bölümü (DEM-74 land sonrası — aynı dosya §8.1.5 çakışma riski). **Faz 3 aktif:** 3A (DEM-42 backend) DEM-74 ile **paralel** başlatılabilir. DEM-74 `In Progress` (kod tab'ı) — kapanınca: `linear-senkron` + DEM-58 epic → `Done` + bu §5 + faz planı Faz 2.7 footer staleness fix | `docs/**` (aktif kod tab'larının dosyaları hariç), `docs/process/05-is-kayit-defteri.md` (koordineli), `.claude/commands/`, Linear |
