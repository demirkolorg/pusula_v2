...
| Faz 3 tab'ları (3A/3B/3C/3D/3E) | `list.move`/`card.move` backend ([DEM-42](https://linear.app/demirkol/issue/DEM-42)) → drag-drop UI web ([DEM-43](https://linear.app/demirkol/issue/DEM-43)) ∥ position compaction worker ([DEM-44](https://linear.app/demirkol/issue/DEM-44)) → Playwright drag-drop testleri ([DEM-45](https://linear.app/demirkol/issue/DEM-45)); `card.moveToList` (cross-board) + `card.copy` ([DEM-69](https://linear.app/demirkol/issue/DEM-69) — Faz 3E, 3A sonrası) | **Geliştirme 2026-05-13 başladı (`faz-baslat 3`)** — epic [DEM-26](https://linear.app/demirkol/issue/DEM-26) `In Progress` (Linear `Sonraki Faz` → `In Progress` senkron düzeltmesi); 3.0 önce-belge ([DEM-41](https://linear.app/demirkol/issue/DEM-41) / `DOC-2026-05-13-001`) `Review` (**bu kontrol odası tab'ında** — `05-board-mekanigi §5.1` + `03-siralama-kurallari` + `08-web §8.1.8` "Board ekranı drag-drop" + `@pusula/domain` move-input şekilleri tamam; önce-belge bitti, kullanıcı onayı bekliyor → onayda `Done`); DEM-74 (Faz 2.7C-2) `Done` (`e9849d2`) — bloklayıcı değil; 3A ([DEM-42](https://linear.app/demirkol/issue/DEM-42)) `Todo` (**başlatmaya hazır** — kod tab'ı); 3B ([DEM-43](https://linear.app/demirkol/issue/DEM-43)) `Todo` — yalnız 3A'ya bağımlı; 3C ([DEM-44](https://linear.app/demirkol/issue/DEM-44)) `Todo` — 3A bekler; 3D ([DEM-45](https://linear.app/demirkol/issue/DEM-45)) `Todo` — 3B bekler; 3E ([DEM-69](https://linear.app/demirkol/issue/DEM-69)) `Todo` — 3A bekler | `packages/api`, `packages/domain`, `packages/db` (3A — gerekirse migration), `apps/worker` (3C), `apps/web` (3B/3D — board kolon bileşenleri/`card-item.tsx`/drag handler'ları/Playwright), `docs/architecture/03,05,06,08` (kod tab'larının kendi turlarında), `docs/domain/01,02,03` |
| **Bu tab (kontrol odası)** | Süreç projeksiyonu + `docs/` + Linear senkronu | sürekli açık; son turlar: `bosluk-tara` 2. tur → 8 yeni issue DEM-66–73; DEM-74 (2.7C-2) tarayıcı görseli ile açıldı + tab prompt'u; `linear-senkron` — DEM-74 → `In Progress`; **`faz-baslat 3` (2026-05-13)** — Faz 3 geliştirmesi başlatıldı: `05-board-mekanigi §5.1` yeniden yazıldı (UI ilkeleri + `moveCard`/`moveList` şekilleri + server move akışı + faz kapsamı + test) + `03-siralama-kurallari` tamamlandı (compaction tetiği `POSITION_COMPACTION_MAX_LEN≈50` + `CONFLICT` semantiği + cross-board `card.moveToList` notu) + faz planı (Faz 3 satırı `🚧` + footer + "Faz 3 alt işleri" 1–6 [DEM-69 madde 6 olarak eklendi]) + iş kayıt defteri (DEM-41 `DOC-2026-05-13-001` `In Progress` + DEM-42–45/69 `API/FE-2026-05-13-*` `Todo` + footer) + bu README §5 (Faz 3 tab'ları satırı) güncellendi; Linear DEM-41 → `In Progress`, DEM-42/43/44/45/69 → `Todo`. `@pusula/domain` move-input şemaları (`moveCardInput`/`moveListInput`) zaten mevcut & doğru — kod yazılmadı. **Kalan:** `08-web §8.1.8` board-drag-drop bölümü (DEM-74 land etti — `e9849d2`; §8.1.5 de wired; §8.1.8 sıradaki). **Faz 3 aktif:** DEM-74 land etti — 3A (DEM-42 backend) ve 3B (DEM-43 UI) artık serbest (3B 3A bekler). DEM-74 (2.7C-2) `Done` — `linear-senkron`/`02-mvp-faz-plani.md`/`05-is-kayit-defteri.md`/`08-web §8.1.5`/`13-ui §13.2 l.165` (wired notu) güncellendi. **`faz3 işlemlerine başla` (2026-05-13):** Faz 3.0 önce-belgenin kalan parçası `08-web §8.1.8` "Board ekranı drag-drop (Faz 3B)" yazıldı (Pragmatic DnD draggable/dropTarget/monitor + `onDrop` → tek `card.move`/`list.move` + optimistic cache/rollback + `CONFLICT` refetch + yetki/arşiv + erişilebilir "Taşı" menüsü; uçtan uca test 3D Playwright'a bırakıldı); `02-mvp-faz-plani.md` (Faz 3 satırı + footer + alt iş 1 — DEM-41 → `Review`, epic DEM-26 → `In Progress`), `05-is-kayit-defteri.md` (DEM-41 satırı + footer), bu README §5 güncellendi; Linear DEM-41 → `Review` + kapanış yorumu, DEM-26 epic → `In Progress` (senkron düzeltmesi). 3A (DEM-42 backend) artık kod tab'ında başlatmaya hazır; kod yazılmadı | `docs/**` (aktif kod tab'larının dosyaları hariç), `docs/process/05-is-kayit-defteri.md` (koordineli), `.claude/commands/`, Linear |
