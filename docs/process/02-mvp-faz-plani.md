...
3. **Workspace davet akışı** — `workspace_invitations` tablosu (süreli, tek kullanımlık token), `members.invite` + `invitations.{list,revoke,mine,accept,decline}` procedure'leri, davet bildirimi (`notification_outbox`); davet akışı kuralları [`../domain/02-yetkilendirme-kurallari.md`](../domain/02-yetkilendirme-kurallari.md) (Workspace davet akışı). — `In Progress` (`API-2026-05-12-002` / [DEM-23](https://linear.app/demirkol/issue/DEM-23); **backend tamam** — db + domain + api + Vitest (46 test), migration `0002`+`0003`, QA (3 reviewer) + verifier PASS, commit `bca834a` pushed; **web davet UI'ı ayrı tur** kaldı).
