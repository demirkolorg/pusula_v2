...
1. **Workspace alanı (backend)** — `workspaceProcedure` middleware (bkz. [`../architecture/03-backend.md`](../architecture/03-backend.md)), Workspace CRUD (create / list / get / update / archive), workspace member listesi + rol yönetimi, permission enforcement (`@pusula/domain/permissions`), ilgili `activity_events`. Procedure → rol haritası: [`../domain/02-yetkilendirme-kurallari.md`](../domain/02-yetkilendirme-kurallari.md). — `Review` (`API-2026-05-12-001`; QA + verifier PASS, commit + kullanıcı onayı bekliyor).
2. **Auth web UI & session** — sign-up / sign-in / sign-out ekranları, session-aware layout, route koruması, workspace listesi/oluşturma ekranı. Bkz. [`../architecture/08-web-ve-mobil.md`](../architecture/08-web-ve-mobil.md). — `Todo`.
