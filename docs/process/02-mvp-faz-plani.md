...
| **1 — Auth & Workspace** | Better Auth sign-in/up/out akışları, session yönetimi, Workspace CRUD, workspace member modeli, permission helper'ları procedure'lere bağlı | Kullanıcı login olup workspace görebilir | ✅ Tamam (alt+ek işler `Done`; küçük UI tamamlayıcı DEM-40 `In Progress`) | _Faz 1_ milestone · [DEM-20](https://linear.app/demirkol/issue/DEM-20) (Done), [DEM-22](https://linear.app/demirkol/issue/DEM-22) (Done), [DEM-23](https://linear.app/demirkol/issue/DEM-23) (Done), [DEM-24](https://linear.app/demirkol/issue/DEM-24) (Done), [DEM-39](https://linear.app/demirkol/issue/DEM-39) (Done), [DEM-40](https://linear.app/demirkol/issue/DEM-40) (In Progress) |
...
6. **`workspace.delete` UI** (ek iş — madde 5'in tamamlayıcısı) — workspace yönetim ekranının tehlikeli bölgesine owner-only "Kalıcı sil" dialog'u: workspace adını birebir yazma onayı (`confirmName`), `trpc.workspace.delete` → `/`'a dön + `workspace.list` invalidate. Bkz. [`../architecture/08-web-ve-mobil.md`](../architecture/08-web-ve-mobil.md) §8.1.2. — `In Progress` (`FE-2026-05-12-004` / [DEM-40](https://linear.app/demirkol/issue/DEM-40); `blockedBy` DEM-24 — backend hazır).

## Faz 2 alt işleri

Faz 2 ([DEM-25](https://linear.app/demirkol/issue/DEM-25) — epic) beş alt işe bölündü (sıra: 2.0 → 2A → (2B ∥ 2C) → 2D).
Linear'da hepsi DEM-25 altında, `Faz 2 — Board/List/Card CRUD` milestone'unda; **DEM-33 (önce belge) `In Progress`**, DEM-34–37 `Sonraki Faz` —
