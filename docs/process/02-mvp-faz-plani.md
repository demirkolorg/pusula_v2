...
| **1 — Auth & Workspace** | Better Auth sign-in/up/out akışları, session yönetimi, Workspace CRUD, workspace member modeli, permission helper'ları procedure'lere bağlı | Kullanıcı login olup workspace görebilir | ✅ Tamam (ek işler `workspace.delete` + workspace yönetim UI `Review`) | _Faz 1_ milestone · [DEM-20](https://linear.app/demirkol/issue/DEM-20) (Done), [DEM-22](https://linear.app/demirkol/issue/DEM-22) (Done), [DEM-23](https://linear.app/demirkol/issue/DEM-23) (Done), [DEM-24](https://linear.app/demirkol/issue/DEM-24) (Review), [DEM-39](https://linear.app/demirkol/issue/DEM-39) (Review) |
...
4. **`workspace.delete`** (ek iş) — owner-only **kalıcı silme** (input'ta workspace adı `confirmName` birebir eşleşmeli; `DELETE FROM workspaces` → üye/davet/board… cascade; `archive`'dan ayrı, geri dönüşsüz; cascade nedeniyle DB içi iz tutulmaz). Bkz. [`../domain/02-yetkilendirme-kurallari.md`](../domain/02-yetkilendirme-kurallari.md). — `Review` (`API-2026-05-12-003` / [DEM-24](https://linear.app/demirkol/issue/DEM-24); kullanıcı kararı: hard delete; backend tamam, +4 test, QA + verifier PASS, commit `7c91527`; kullanıcı onayı bekliyor).
5. **Workspace yönetim ekranı (web)** (ek iş) — backend'de var olan `workspace.update`/`archive` + `workspace.members.{list,updateRole,remove}` + `workspace.invitations.{list,revoke}` procedure'lerini UI'ye bağlar: route `(app)/workspaces/[id]` — rename/slug + arşivle + üye listesi/rol/çıkar/ayrıl + gönderilmiş davetler + davet et (rol-bazlı görünürlük). Bkz. [`../architecture/08-web-ve-mobil.md`](../architecture/08-web-ve-mobil.md) §8.1.2. — `Review` (`FE-2026-05-12-003` / [DEM-39](https://linear.app/demirkol/issue/DEM-39); implemente edildi — page + alt component'ler + `@pusula/ui` `Select`; QA + verifier PASS, commit `7c91527`; kullanıcı onayı bekliyor).
