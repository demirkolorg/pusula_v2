...
> henüz başlamamış "bir sonraki faz"ın alt işleri Linear'da `Backlog` durumunda + `sonraki-faz` label'ı taşır
> (Backlog ↔ Todo arası ara katman: `Backlog` = uzak, `sonraki-faz` = bir sonraki faz/planlı, `Todo` = mevcut fazın bekleyen işleri); faz başlayınca `Todo`'ya alınır ve label kaldırılır.
...
3. **Workspace davet akışı** — `workspace_invitations` tablosu (süreli, tek kullanımlık token), `members.invite` + `invitations.{list,revoke,mine,accept,decline}` procedure'leri, davet bildirimi (`notification_outbox`); davet akışı kuralları [`../domain/02-yetkilendirme-kurallari.md`](../domain/02-yetkilendirme-kurallari.md) (Workspace davet akışı). — `In Progress` (`API-2026-05-12-002` / [DEM-23](https://linear.app/demirkol/issue/DEM-23); **backend tamam** — db + domain + api + Vitest, migration `0002_omniscient_lady_ursula.sql`, `typecheck`/`lint`/`test` yeşil; **web davet UI'ı ayrı tur** kaldı).
...
Linear'da hepsi DEM-25 altında, `Faz 2 — Board/List/Card CRUD` milestone'unda; durum `Backlog` + `sonraki-faz` label'ı —
yani **Backlog ile Todo arasında ara katman** ("bir sonraki faz, planlı"). Faz 2 geliştirmesi başlayınca her alt iş `Todo`'ya
alınır ve label kaldırılır. İş kayıt defteri satırları Faz 2.0 işinde eklenir ([`05-is-kayit-defteri.md`](05-is-kayit-defteri.md)).
