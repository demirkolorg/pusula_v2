...
description: 'Codex için Pusula çalışma protokolü ve dokümantasyon yönlendiricisi.'
aliases:
  - 'Çalışma Protokolü'
  - 'Codex Protokolü'
tags:
...
# Pusula — Çalışma Protokolü (Codex)
...
İmplementasyon sözleşmesi (Codex skill): [`.Codex/skills/kontrol/SKILL.md`](.Codex/skills/kontrol/SKILL.md)
...
| `apps/mobile`     | `@pusula/mobile`     | Expo (SDK 54) + React Native + expo-router + NativeWind; board/kart/bildirim ekranları, optimistic UI, push (**App Store'da yayında**). Mobil etkileşim/motion **ana kuralı**: Reanimated token + primitive katmanı → `docs/architecture/20-hareket-etkilesim-sistemi.md` §20.11 | Ana API mantığı (`@pusula/api`), push/email gönderimi (worker), başka UI library | `docs/architecture/08-web-ve-mobil.md`, `20-hareket-etkilesim-sistemi.md` §20.11 |
...
- **Çelişki:** Gelen istek mevcut belge (`docs/`, `.Codex/skills/kontrol/SKILL.md`) ile çelişiyorsa, işe başlamadan bana bildir ve "Belgeyi mi güncelleyelim, koda mı sadık kalalım?" diye sor.
- **İnce tut:** Kök AGENTS.md ve `.Codex/skills/kontrol/SKILL.md` ince kalsın — ayrıntı `docs/`'a, buraya sadece özet + pointer. README + skill, `docs/` yapısıyla tutarlı kalmalı.
