...
describe('searchDocuments table', () => {
  it('matches the Faz 6.5 FTS column contract', () => {
    const columns = getTableColumns(searchDocuments);
...
describe('DEM-104 migration', () => {
  it('activates search_documents without dropping existing data', () => {
    const migrationPath = resolve(import.meta.dirname, '../../drizzle/0016_icy_dark_phoenix.sql');
    ...
    expect(migration).toContain(
      `ALTER TABLE "search_documents" ALTER COLUMN "labels" SET DATA TYPE text[]`,
    );
    expect(migration).toContain(
