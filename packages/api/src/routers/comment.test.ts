...
describe.runIf(dbAvailable)('comment router (integration)', () => {
  ...
  it('create: a member adds a comment; comment.created activity; boards.version bumps; a board viewer is FORBIDDEN', async () => {
    ...
    await expect(
      callerFor(guestId).comment.create({ cardId, body: 'Nope', clientMutationId: newId('cmid') }),
  ...
  it('list: returns a card\'s comments in ascending created_at order, including soft-deleted (empty-body) rows', async () => {
    const a = await callerFor(memberId).comment.create({ cardId, body: 'alpha', clientMutationId: newId('cmid') });
    const b = await callerFor(ownerId).comment.create({ cardId, body: 'bravo', clientMutationId: newId('cmid') });
    await callerFor(memberId).comment.delete({ cardId, commentId: a.id, clientMutationId: newId('cmid') });
  ...
  it('update: the author edits a comment (comment.updated, version+1); same body is idempotent; a non-author non-admin is FORBIDDEN; a board admin may edit it; a viewer is FORBIDDEN', async () => {
    const c = await callerFor(memberId).comment.create({ cardId, body: 'editable', clientMutationId: newId('cmid') });
    ...
    await expect(
      callerFor(guestId).comment.update({ cardId, commentId: c.id, body: 'hax', clientMutationId: newId('cmid') }),
    ).rejects.toMatchObject({ code: 'FORBIDDEN' });
  ...
  it('update: editing a soft-deleted comment is BAD_REQUEST', async () => {
    const c = await callerFor(memberId).comment.create({ cardId, body: 'doomed', clientMutationId: newId('cmid') });
    await callerFor(memberId).comment.delete({ cardId, commentId: c.id, clientMutationId: newId('cmid') });
    await expect(
      callerFor(memberId).comment.update({ cardId, commentId: c.id, body: 'zombie', clientMutationId: newId('cmid') }),
  ...
  it('delete: the author soft-deletes (deletedAt set, body cleared, comment.deleted, version+1); idempotent; a non-author non-admin is FORBIDDEN; a board admin may delete it', async () => {
    const c = await callerFor(memberId).comment.create({ cardId, body: 'remove me', clientMutationId: newId('cmid') });
    ...
    // idempotent
    const noop = await callerFor(memberId).comment.delete({ cardId, commentId: c.id, clientMutationId: newId('cmid') });
    expect(noop).toMatchObject({ id: c.id, changed: false });

    // a plain member cannot delete someone else's comment; a board admin can
    const byAdmin = await callerFor(adminId).comment.create({ cardId, body: 'admin owns this', clientMutationId: newId('cmid') });
    await expect(
      callerFor(memberId).comment.delete({ cardId, commentId: byAdmin.id, clientMutationId: newId('cmid') }),
    ).rejects.toMatchObject({ code: 'FORBIDDEN' });
  ...
  it('NOT_FOUND: a commentId belonging to another card cannot be updated or deleted via this card', async () => {
    ...
    await expect(
      callerFor(ownerId).comment.update({ cardId, commentId: onOther.id, body: 'hax', clientMutationId: newId('cmid') }),
    ).rejects.toMatchObject({ code: 'NOT_FOUND' });
    await expect(
      callerFor(ownerId).comment.delete({ cardId, commentId: onOther.id, clientMutationId: newId('cmid') }),
    ).rejects.toMatchObject({ code: 'NOT_FOUND' });
  });
  ...
  it('an outsider (not a workspace member) can neither list nor create comments (FORBIDDEN)', async () => {
    ...
    await expect(
      callerFor(outsiderId).comment.create({ cardId, body: 'sneaky', clientMutationId: newId('cmid') }),
    ).rejects.toMatchObject({ code: 'FORBIDDEN' });
  });
  ...
  it('create: an archived board rejects new comments (BAD_REQUEST)', async () => {
    ...
    await expect(
      callerFor(ownerId).comment.create({ cardId: cardOnOther.id, body: 'Nope', clientMutationId: newId('cmid') }),
    ).rejects.toMatchObject({ code: 'BAD_REQUEST' });
  });
});
