...
describe('<WorkspaceSettingsForm>', () => {
  it('renders name and slug fields pre-filled with the current values', () => {
    render(
      <WorkspaceSettingsForm name="Pazarlama Ekibi" slug="pazarlama-ekibi" onSubmit={vi.fn()} />,
    );
  ...
  it('disables submit until something changed', async () => {
    ...
    render(<WorkspaceSettingsForm name="Ekip" slug="ekip" onSubmit={vi.fn()} />);
  ...
  it('blocks submit and shows field errors on an invalid name and slug', async () => {
    ...
    render(<WorkspaceSettingsForm name="Ekip" slug="ekip" onSubmit={onSubmit} />);
  ...
  it('calls onSubmit with the trimmed values when valid', async () => {
    ...
    expect(onSubmit).toHaveBeenCalledWith({ name: 'Yeni Ekip', slug: 'yeni-ekip' });
  });
