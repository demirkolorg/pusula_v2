      ...
      <div className="relative z-10 flex h-full w-full flex-col justify-between p-10 xl:p-14">
        ...
        <div className="flex flex-col gap-6">
          <h2 className="text-4xl/[1.05] font-semibold tracking-tight xl:text-5xl/[1.05]">
            ...
            <br /> <span className="italic opacity-90">{copy.headline.emphasis}</span>{' '}
        ...
        <div className="flex flex-col gap-3">
          <SummaryCard summary={copy.summary} />
          <p className="text-xs opacity-60">{copy.footer}</p>
        </div>
      </div>
    </aside>
  );
...
type SummaryCopy = typeof strings.auth.brandPanel.summary;

function SummaryCard({ summary }: { summary: SummaryCopy }) {
  return (
    <div className="rounded-2xl border border-white/15 bg-white/10 p-4 shadow-[0_20px_40px_-20px_rgb(0_0_0_/_0.5)] backdrop-blur-md">
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-[11px] uppercase tracking-wider opacity-60">{summary.label}</p>
          <p className="mt-0.5 text-base font-medium">{summary.title}</p>
        </div>
      ...
      <dl className="mt-4 grid grid-cols-3 gap-3 text-center">
        {summary.metrics.map((metric) => (
          <Metric key={metric.title} title={metric.title} value={metric.value} />
        ))}
      </dl>
    </div>
  );
